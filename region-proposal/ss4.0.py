...
主功能：框选目标区域
'''
def detect(original_folder_path, filename, processed_folder_path, result_folder_path):
    # 加载图像
    image_path = original_folder_path + "/" + filename
...
if __name__ == "__main__":
    original_folder_path = "data/capture_image"
    processed_folder_path = "data/ss4_result/processed_image"
    result_folder_path = "data/ss4_result/final_result"
    if not os.path.exists(processed_folder_path):
        os.makedirs(processed_folder_path)
    if not os.path.exists(result_folder_path):
        os.makedirs(result_folder_path)
    filename_list = get_jpg_files(original_folder_path)
    print(filename_list)
    for i in range(0,len(filename_list)):
        start_time = datetime.now()
        detect(original_folder_path, filename_list[i], processed_folder_path, result_folder_path)
