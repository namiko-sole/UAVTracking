...
# PID控制器类
from tools.realtime import Realtime
# 模拟器类
class DroneSimulator:
    def __init__(self, map_path, window_size, tracking_area, ship_speed, pid_params):
        ...
        # 初始化PID控制器
        self.realtime = Realtime(pid_params)
    ...
    def updateNew(self,frame):
        """
        根据船和无人机的速度更新位置，并调整滑动窗口位置。
        """
        # 更新无人机速度（PID控制）
        self.drone_speed[0],self.drone_speed[1],dt = self.realtime.trackWithRect(frame)
